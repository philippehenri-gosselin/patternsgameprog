...
public class WorldTest {
    ...
    public void testAccess() {
        ...
        int x = 1;
        int y = 1;
        world.set(x,y-1,new Wall(WallTypeId.BOTTOMRIGHT));
    ...
    public void testIterator() {
        ...
        for (int y=0;y<height;y++) {
            for (int x=0;x<width;x++) {
                if (rand.nextBoolean()) {
                    tab[x][y] = new Space(SpaceTypeId.GUM);
                }
                else {
                    tab[x][y] = new Wall(WallTypeId.TOPLEFT);
                }
            }
        }
